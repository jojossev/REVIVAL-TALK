...
class CategoryController extends Controller
{
    ...
    public function show(Request $request)
    {
        ...
        // news count category wise
        $sql = Category::with('language')->withCount('news');
        ...
        $rows = $sql->get()->map(function ($row) {
            ...
            if (auth()->user()->can('category-delete')) {
                    $delete = '<a data-url="' . url('category', $row->id) . '" class="dropdown-item delete-form" data-id="' . $row->id . '" title="' . __('delete') . '" data-title="' . __('are_you_sure') . '" data-text="' . __('deleting_a_category_subcategory_or_tag_automatically_deletes_related_news_without_additional_confirmation') . '"><i class="fa fa-trash mr-1 text-danger"></i>' . __('delete') . '</a>';
            }
            if ($edit == '' && $delete == '') {
                $operate = '-';
            }
    ...
    public function update(Request $request)
    {
        ...
        $category->language_id = $request->language;
        $category->category_name = $request->name;
        $category->slug = generateUniqueSlug($request->slug);
        if ($request->hasFile('file')) {
