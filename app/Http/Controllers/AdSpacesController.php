    ...
    public function show(Request $request)
    {
        ...
        $rows = $sql->get()->map(function ($row) {
            ...
            return [
                ...
                'ad_url' => $row->ad_url ?? '',
                'status1' => $row->status ? "<span class='badge badge-success'>". __('active') . "</span>" : "<span class='badge badge-danger'>". __('deactive') . "</span>",
                'status' => $row->status,
                'operate' => $operate,
            ];
        });
        ...
        if (!empty($res)) {
            foreach ($res as $value) {
                if ($value['style_app'] == 'style_6' || $value['style_web'] == 'style_web') {
                    $option .= '<option value="featuredsection-'.$value['id'].'" >'.__('above'). ' ' . $value['title'] . '.  (' . __('not_applicable') . ')</option>';
                } else {
                    $option .= '<option value="featuredsection-'.$value['id'].'">'.__('above') . ' ' . $value['title'] . '</option>';
                }
            }
        }
        $option .= '<option value="news_details_top">' . __('news_details_top') . '</option>';
        $option .= '<option value="news_details_bottom">' . __('news_details_bottom') . '</option>';
        return $option;
    }
}
