        ...
        if (settingsData?.data?.lat || currentPage || slug) {
            ...
            try {
                ...
                if (!data?.error) {
                    ...
                    setTotalData(data?.data?.data[0]?.news ? data?.data?.data[0]?.news_total : data?.data?.data[0]?.breaking_news ? data?.data?.data[0]?.breaking_news_total : data?.data?.data[0]?.videos_total)

                    if (loadMore) {
                        if (data?.data?.data[0]?.news) {
                            setViewAllData((prevData) => [...prevData, ...data?.data?.data[0]?.news])
                        }
                        else if (data?.data?.data[0]?.videos) {
                            setIsVideoNews(true)
                            setViewAllData((prevData) => [...prevData, ...data?.data?.data[0]?.videos])
                        }
                        else {
                            setIsBreakingNews(true)
                            setViewAllData((prevData) => [...prevData, ...data?.data?.data[0]?.breaking_news])
                        }
                    }
                    else {
                        if (data?.data?.data[0]?.news) {
                            setViewAllData(data?.data?.data[0]?.news);
                        }
                        else if (data?.data?.data[0]?.videos) {
                            setIsVideoNews(true)
                            setViewAllData(data?.data?.data[0]?.videos);
                        }
                        else {
                            setIsBreakingNews(true)
                            setViewAllData(data?.data?.data[0]?.breaking_news);
                        }
                    }
