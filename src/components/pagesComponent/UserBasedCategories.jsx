...
import { NoDataFound, placeholderImage, setCateOffset,  defaultLanguageCode, currentLangCode } from '@/utils/helpers';
...
const UserBasedCategories = () => {
    ...
    useEffect(() => {
        const toggledData = categories?.map(element => {
            // Debug logging
            // console.log('element id:', element.id, 'type:', typeof element.id);
            
            // Handle category_id as a string "23,25" and convert to array of numbers
            const categoryIdString = userData?.userManageData?.data?.user_category?.category_id || '';
            const categoryIds = categoryIdString.toString().split(',').map(id => Number(id.trim()));
            // console.log('category_ids after split:', categoryIds);

            const elementId = Number(element.id);
            const isToggledOn = categoryIds.includes(elementId);
        ...
        // console.log('final toggled data:', toggledData)

    }, [currentLanguage?.id, categories, cateLimit, totalCategories])
                                                ...
                                                return <div className='flex items-center justify-between border borderColor rounded-[10px] p-2 gap-3 sm:p-4 sm:gap-0 md:p-2 md:gap-3 lg:p-4 lg:gap-0' key={element?.id}>
                                                    ...
                                                    <div>
                                                        ...
                                                        >
                                                            {/* Slider */}
                                                            <div
                                                                className={`absolute ltr:left-1 rtl:right-1 top-1 h-[80%] w-[26px] commonRadius bg-white border-2 transition-transform duration-300 transform ${element?.isToggledOn ? 'ltr:translate-x-[46px] rtl:-translate-x-[46px] ltr:sm:translate-x-[65px] rtl:sm:-translate-x-[65px] ltr:md:translate-x-[46px] rtl:md:-translate-x-[46px] ltr:lg:translate-x-[65px] rtl:lg:-translate-x-[65px]' : 'translate-x-0'
                                                                    }`}
                                                            ></div>
                                                            <span className={`absolute ltr:right-[12px] rtl:left-[12px] font-semibold text-white text-sm transition-opacity duration-300 ${element?.isToggledOn ? 'opacity-0' : 'opacity-100'}`}>{translate('off')}</span>
                                                            <span className={`absolute ltr:left-[12px] rtl:right-[12px] font-semibold text-white text-sm transition-opacity duration-300 ${element?.isToggledOn ? 'opacity-100' : 'opacity-0'}`}>{translate('on')}</span>
                                                        </div>
                                                    </div>
                                                </div>
                                            })
