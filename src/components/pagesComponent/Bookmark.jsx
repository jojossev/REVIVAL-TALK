...
const Bookmark = () => {
    ...
    const getbookmarkApi = async () => {
        ...
        try {
            ...
            if (!data?.error) {
                if (loadMore) {
                    setBookmarkData((prevData) => [...prevData, ...data?.data?.data])
                }
                else {
                    setBookmarkData(data?.data?.data)
                }
                        ...
                        isLoading?.loading ? <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 gap-y-10'>
                            ...
                            : bookmarkData && bookmarkData?.length > 0 ? <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 gap-y-10'>
                                ...
                                <div className='flexColumnCenter gap-2'>
                                    <div>
                                        <Image src={bookmarkImg} height={0} width={0} className='h-auto w-[200px] md:w-[250px] dark:brightness-[0] dark:invert-[1]' onError={placeholderImage} alt={'no bookmark'} loading='lazy' />
                                    </div>
                                    <div>
