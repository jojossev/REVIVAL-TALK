...
const SubCategoryNews = () => {
        ...
        if (settingsData?.data?.lat || currentPage || slug) {
            ...
            try {
                ...
                if (!data?.error) {
                    ...
                    if (loadMore) {
                        setSubsubCategoriesNewsData((prevData) => [...prevData, ...newsData])
                    }
                    else {
                        setSubsubCategoriesNewsData(newsData);
                    }
                }
                else {
    ...
    useEffect(() => {
        fetchNews()
    }, [currentLanguage?.code, settingsData, slug, offset])
                            ...
                            isLoading.loading ? [...Array(4)].map((_, index) => (
                                ...
                                subCategoriesNewsData?.map((element) => {
                                    return <div key={element?.id}>
                                        <NewsCard element={element} subCateSlug={slug} />
                                    </div>
                                })
                        }
                        <div>
