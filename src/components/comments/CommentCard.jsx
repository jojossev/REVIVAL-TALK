...
import { isLogin, placeholderImage, truncateText } from '@/utils/helpers';
...
const CommentCard = ({ repliedCard, isLoading, element, getCommentByNews, newsId, isCommentEnabled }) => {
    ...
    const [showFullMessage, setShowFullMessage] = useState(false) // State to track if full message is shown

    const [replyComment, setReplyComment] = useState("");

    // Check if message is longer than 800 characters
    const isMessageLong = element?.message && element.message.length > 800

    // Function to toggle between truncated and full message
                ...
                <div className='flex flex-col gap-2 md:gap-4 border borderColor commonRadius p-2 textPrimary w-full'>
                    ...
                    <div>
                        <span className='text-[14px] md:text-[16px]'>
                            {showFullMessage ? element?.message : truncateText(element?.message, 800)}
                        </span>
                        {isMessageLong && (
                            <button 
                                onClick={toggleMessageView}
                                className='primaryColor text-[12px] md:text-[14px] font-semibold ml-2 underline'
                            >
                                {showFullMessage ? translate('readLess') || 'Read Less' : translate('readMore') || 'Read More'}
                            </button>
                        )}
                    </div>
                    <div className='flex items-center justify-between relative'>
...

                            {
                                userData?.data?.id === element?.user_id ? <span className='cursor-pointer' onClick={e => deleteComment(e, element?.id)}>
                                    ...
                                    <Dialog open={reportModal} >
                                        ...
                                        <DialogContent className='w-[300px] sm:w-full bodyBgColor reportModal'>
                                            <div className='reportCard mt-6'>
                                                ...
                                                <div className='mt-2'>
                                                    <textarea name="report" value={reportMsg} onChange={(e) => setReportMsg(e.target.value)} id="report" cols="60" rows={'6'} className='w-full border borderColor commonRadius focus:outline-none p-2 dark:text-black'></textarea>
                                                </div>
                        ...
                        <div>
                            <Popover open={replyModal}>
                                ...
                                <PopoverContent className='absolute top-2 -right-12 sm:-right-8 bodyBgColor' onMouseLeave={() => setReplyModal(false)}>
