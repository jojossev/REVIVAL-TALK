...
import { currentLangCode, getAuthErrorMessage, } from '@/utils/helpers';
        ...
        try {
            ...
            if (response?.data?.error) {
                ...
                if (response?.data?.data?.fcm_id && currentLanguage?.id) {
                    setTimeout(async () => {
                        await registerToken(response?.data?.data?.fcm_id,response?.data?.data?.id)
                    }, [1000])
                }
                if (response?.data?.data?.is_login === '0') {
