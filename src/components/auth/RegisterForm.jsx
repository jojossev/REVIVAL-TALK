...
const RegisterForm = ({ setLoginModal, setRegisterModal }) => {
    ...
    const togglePasswordVisibility = (input) => {
        if (input === 'pass') {
            setPasswordVisible({ ...isPasswordVisible, pass: !isPasswordVisible.pass });
        }
        if (input === 'confPass') {
            setPasswordVisible({ ...isPasswordVisible, confPass: !isPasswordVisible.confPass });
        }
    };
    ...
    const handleSignup = async (email, password) => {
        ...
        if (Object.keys(formErrors).length === 0) {
            try {
                ...
                toast.success(`${translate('verifSentMail')}${formValues.email}`)
