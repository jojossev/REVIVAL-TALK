            ...
            <NavigationMenuList>
                <NavigationMenuItem>
                    <NavigationMenuTrigger className="flexCenter rtl:flex-row-reverse py-2 px-3 gap-1 dropdownTrigg commonRadius commonDropDown commonBg dark:bg-white dark:text-black w-max text-[17px] !font-[500]">
                        <Image src={userData?.data?.profile ? userData?.data?.profile : profileImg} height={24} width={24} alt='profile' className='rounded-full me-1 w-[24px] h-[24px] object-cover' onError={placeholderImage} loading='lazy' />
                        {truncateText(userData.userName ? userData.userName : userName, 10)}
                    </NavigationMenuTrigger>
