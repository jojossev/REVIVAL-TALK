...
const WithoutSeoShare = ({ url }) => {
    ...
    // Direct sharing URLs
    const shareUrls = {
        facebook: `https://www.facebook.com/sharer/sharer.php?u=${decodeURI(finalUrl)}`,
        whatsapp: `https://api.whatsapp.com/send?text=${decodeURI(finalUrl)}`,
        twitter: `https://twitter.com/intent/tweet?url=${decodeURI(finalUrl)}`,
        addToAny: `https://www.addtoany.com/share#url=${decodeURI(finalUrl)}`
    };
    ...
    const handleCopyUrl = async (e) => {
        ...
        try {
            ...
            await navigator.clipboard.writeText(decodeURI(finalUrl));
            toast.success(translate('urlCopied'));
        } catch (error) {
