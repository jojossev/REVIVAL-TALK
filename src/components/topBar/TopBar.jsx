...
import { currentLangCode, defaultLanguageCode, isLogin, } from '@/utils/helpers'
import { checkLocationPermissionGranted, checkPermissionsSelector, isLocationPermissionCheck, isNotificationPermissionCheck, notificationRequestSelector } from '../store/reducers/CheckPermissionsReducer'
...
const TopBar = ({ setMorePagesOffset, setIsLoadMorePages }) => {
  ...
  }, [settingsData?.fcmtoken, notificationPermission])

  useEffect(() => {
    if (checkPermissions?.data?.isLocationPermission === 'granted' && isLogin() && !checkLocationPermissonOnce) {
      registerToken('')
