...
export const placeholderImage = (e) => {
  ...
  const placeholderSrc = isDarkMode ? settingsData?.data?.web_setting?.dark_placeholder_image : settingsData?.data?.web_setting?.light_placeholder_image
...
export const defaultLanguageCode = () => {
  return store.getState().settings?.data?.default_language?.code
}

// Validate slug format - must be English only with hyphens
export const validateSlugFormat = (slug) => {
  const englishSlugPattern = /^[a-z0-9]+(?:-[a-z0-9]+)*$/;
  return englishSlugPattern.test(slug);
};

export const decryptGeminiKey = (apiKeyString, decryptionKey = null) => {

  try {
    const base64Decoded = atob(apiKeyString);
    // console.log('Result:', base64Decoded);
    // console.log('Looks like API key:', base64Decoded.length > 10 && (base64Decoded.includes('AIza') || base64Decoded.includes('-') || base64Decoded.includes('_')));
    // console.log('');

    try {
      const jsonParsed = JSON.parse(base64Decoded);

      // If it has ciphertext and iv, try AES decryption
      if (jsonParsed.ciphertext && jsonParsed.iv && decryptionKey) {

        import('crypto-js').then(cryptoJs => {
          let encrypted = jsonParsed.ciphertext;
          let iv = cryptoJs.enc.Hex.parse(jsonParsed.iv);
          let key = decryptionKey + "0000";
          ...
          console.log('CryptoJS not available, install with: npm install crypto-js');
        });
      }

    } catch (e) {
      // console.log('Is JSON:', false);
      // console.log('Direct use result:', base64Decoded);
    }
...
export const resizeImageFixed = (file, targetWidth, targetHeight, quality = 1.0) => {
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      // draw image stretched or squeezed to fixed size
      ctx.drawImage(img, 0, 0, targetWidth, targetHeight);

      canvas.toBlob(
        (blob) => {
          const resizedFile = new File([blob], file.name, {
            type: file.type,       // keep original (or change to webp if you want)
